...
public class MetricService implements IMetricService {
    ...
    private static final Integer STATUS_CODE_METRICS  = 200;
    ...
    @Override
    public Map getCurrentRequestMetrics() {
        ...
        for (final Entry<String, ConcurrentHashMap<Integer, Integer>> entry : timeMap.entrySet()) {
             ...
             String requestTime = entry.getKey();
             tempMap = entry.getValue();
             Integer requestCounts = tempMap.get(STATUS_CODE_METRICS);//Get the records whose status is success
             if (requestCounts == null) {
            	 requestCounts = 0;
             }
                result.put(requestTime, requestCounts);
        }
